...
public class PathKit {
  ...
  public static String getRootClassPath() {
    if (rootClassPath == null) {
      try {
        String path = PathKit.class.getClassLoader().getResource("").toURI()
            .getPath();
  ...
  public static String getPackagePath(Object object) {
    Package pg = object.getClass().getPackage();
    return pg != null ? pg.getName().replaceAll("\\.", "/") : "";
  }
  ...
  /*
   * private static String detectWebRootPath() { try { String path =
   * PathKit.class.getResource("/").getFile(); return new
   * File(path).getParentFile().getParentFile().getCanonicalPath(); } catch
